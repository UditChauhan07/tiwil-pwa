...
const SOCKET_SERVER_URL = "http://45.77.110.58:6543";
...
const ChatRoom = () => {
  ...
  useEffect(() => {
    ...
    socketRef.current = io(SOCKET_SERVER_URL, {
      ...
      query: { token }, // Send the token in the query
    });
                ...
                {msg.messageType === "image" && msg.mediaUrl && (
                  <img
                    src={
                      msg.isOptimistic
                        ? msg.mediaUrl
                        : `${BASE_API_URL}${msg.mediaUrl.replace(/\\/g, "/")}`
                    }
                    alt="Chat attachment"
      ...
      <div className={styles.inputContainer}>
        ...
        {mediaFile && (
          <div className={styles.mediaPreview}>
            <button className={styles.removeMedia} onClick={() => setMediaFile(null)}>
              ❌
            </button>
          </div>
        )}
        ...
        <input
          ...
          onKeyDown={(e) =>
            e.key === "Enter" && !e.shiftKey ? (e.preventDefault(), handleSendMessage()) : null
          }
