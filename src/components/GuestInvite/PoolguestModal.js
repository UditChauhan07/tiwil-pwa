...
const InviteModal = ({ show, setShow }) => {
  ...
  const eventId = localStorage.getItem("eventId");  // or use useParams() if eventId is in URL
  ...
  useEffect(() => {
    const fetchGuests = async () => {
      try {
        ...
        setUsers(response.data); // Corrected from setGuest to setUsers
      } catch (error) {
    ...
    fetchGuests();
  }, [eventId]);  // Dependency on eventId, refetch if it changes
  ...
  ? users.filter((user) =>
      user.fullName && user.fullName.toLowerCase().includes(searchQuery.toLowerCase())  // Ensure fullName exists
    )
  ...
  const handleSendInvite = async () => {
    try {
      ...
      const inviteData = {
        ...
        eventId, // Use the actual eventId here
      };
    ...
    <div>
      ...
      <Modal show={show} onHide={() => setShow(false)} centered size="md">
        <Modal.Header closeButton>
          <Modal.Title>Select Guests to Invite to Pool</Modal.Title>
        </Modal.Header>
